    ...
    <ScrollView style={styles.leftPanel}>
        ...
        <ScrollView horizontal>
          {otherImages.map((item, index) => (
            <TouchableOpacity key={index} style={styles.otherImageContainer} onPress={() => handleOzetPress(item)}>
              ...
              <Text style={styles.summary}>{item.summary.slice(0, 100)}{item.summary.length > 100 ? '... (devamını oku)' : ''}</Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
      </View>
      ...
      <ScrollView style={styles.additionalImagesContainer}>
        {additionalImages.map((item, index) => (
          <TouchableOpacity key={index} style={styles.additionalImageContainer} onPress={() => handleOzetPress(item)}>
